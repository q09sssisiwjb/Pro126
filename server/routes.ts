...
Return only the enhanced prompt, nothing else.`;
  ...
  // Sketch-to-image generation endpoint using gemini-2.0-flash-exp
  app.post("/api/sketch-to-image", async (req, res) => {
    try {
      ...
      // Process sketch data to remove data URL prefix if present
      const processedSketchData = sketchData.includes(',') ? sketchData.split(',')[1] : sketchData;

      // Build the transformation prompt based on whether user provided a custom prompt
      const transformPrompt = prompt 
        ? `Convert this sketch into a photorealistic image based on the following description: ${prompt}. Transform the hand-drawn lines and shapes into a detailed, realistic photograph with proper lighting, textures, and colors that matches the description. Make it look like a real-life photograph.`
        : 'Convert this sketch into a photorealistic image. Transform the hand-drawn lines and shapes into a detailed, realistic photograph with proper lighting, textures, and colors. Make it look like a real-life photograph.';

      const result = await generateImageToImage(
        [{ data: processedSketchData, type: 'image/png' }],
        transformPrompt
      );

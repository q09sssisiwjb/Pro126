...
// User Custom Skies table
export const userCustomSkies = pgTable("user_custom_skies", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: text("user_id").notNull(), // Firebase user ID
  name: text("name").notNull(),
  description: text("description"),
...
export const insertUserCustomSkySchema = createInsertSchema(userCustomSkies).pick({
  userId: true,
  name: true,
  description: true,
  atmosphere: true,
  ...
  colorPalette: z.string().optional().nullable().default(null),
  weatherConditions: z.string().optional().nullable().default(null),
  timeOfDay: z.string().optional().nullable().default(null),
});

export type InsertUserCustomSky = z.infer<typeof insertUserCustomSkySchema>;
export type UserCustomSky = typeof userCustomSkies.$inferSelect;
